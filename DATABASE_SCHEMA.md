...
Aplikasi ini menggunakan **SQLite** sebagai sistem manajemen basis data. Di bawah ini adalah rincian setiap tabel, kolom, tipe data, dan relasinya.
...
| :--- | :--- | :--- |
| `id` | INTEGER (PK) | Auto-increment ID. |
| `item_id` | INTEGER (FK) | Relasi ke `items.id`. |
| `location_id` | INTEGER (FK) | Relasi ke `locations.id`. |
...
| :--- | :--- | :--- |
| `id` | INTEGER (PK) | Auto-increment ID. |
| `user_id` | INTEGER (FK) | ID staff yang mencatat (FK ke `users.id`). |
| `borrower_name` | TEXT | Nama orang yang meminjam. |
| `borrower_phone` | TEXT | Nomor WhatsApp peminjam. |
| `start_date` | DATETIME | Tanggal mulai pinjam. |
| `due_date` | DATETIME | Batas waktu pengembalian. |
...
| :--- | :--- | :--- |
| `id` | INTEGER (PK) | Auto-increment ID. |
| `borrowing_id` | INTEGER (FK) | Relasi ke `borrowings.id`. |
| `item_id` | INTEGER (FK) | Relasi ke `items.id`. |
| `location_id` | INTEGER (FK) | Lokasi pengambilan barang. |
...
| `return_date` | DATETIME | Tanggal barang dikembalikan. |
...
| `id` | INTEGER (PK) | Auto-increment ID. |
| `item_id` | INTEGER (FK) | Relasi ke `items.id`. |
| `location_id` | INTEGER (FK) | Lokasi barang saat ditemukan rusak. |
| `quantity` | INTEGER | Jumlah barang yang rusak. |
| `cause` | TEXT | Penyebab kerusakan. |
| `damage_date` | DATETIME | Tanggal kejadian kerusakan. |
...
| :--- | :--- | :--- |
| `id` | INTEGER (PK) | Auto-increment ID. |
| `user_id` | INTEGER (FK) | ID pengguna yang melakukan aksi. |
| `action` | TEXT | Jenis aksi (misal: `LOGIN`, `CREATE_ITEM`). |
| `details` | TEXT | Rincian keterangan aksi. |
| `created_at` | DATETIME | Waktu aktivitas tercatat. |
